...
import { Users, FileText, Activity, ShieldAlert, TrendingUp, Loader2, ArrowLeft } from 'lucide-react';
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-100 font-inter p-8 transition-colors duration-300">
      <div className="max-w-7xl mx-auto">
        <header className="flex justify-between items-center mb-10 border-b border-slate-200 dark:border-slate-800 pb-6">
            <div>
                <h1 className="text-3xl font-bold flex items-center gap-3 text-red-600 dark:text-red-500">
                    <ShieldAlert /> CENTRO DE COMANDO
                </h1>
                <p className="text-slate-500 dark:text-slate-400 mt-1">Visão Geral do Sistema & Gestão de Usuários</p>
            </div>
            <Link to="/dashboard" className="bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 px-6 py-2 rounded-lg text-sm font-bold transition border border-slate-200 dark:border-slate-700 shadow-sm flex items-center gap-2">
                <ArrowLeft size={16}/> Sair para o App
            </Link>
        </header>
