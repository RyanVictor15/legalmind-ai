...
export default function Login() {
  ...
  const [step, setStep] = useState(1); // 1 = Senha, 2 = 2FA
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      console.error(err);
      const msg = err.response?.data?.message || "Erro ao conectar.";
      toast.error(msg);
    } finally {
      setLoading(false);
    }
      ...
      <div className="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-md border border-slate-800">
        ...
        <div className="text-center mb-8">
          ...
          <h2 className="text-3xl font-bold text-slate-800">
            {step === 1 ? 'Acessar Conta' : 'Segurança'}
          </h2>
          <p className="text-slate-500 mt-2">
            {step === 1 ? 'Bem-vindo ao LegalMind AI' : `Digite o código enviado para ${email}`}
          </p>
        </div>
        ...
        {step === 1 && (
            ...
            Não tem conta? <Link to="/register" className="text-blue-600 font-bold hover:underline">Registre-se grátis</Link>
            </p>
        )}
      </div>
    </div>
